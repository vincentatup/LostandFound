...
class LoginDialog(QDialog):
    ...
    def setup_login_tab(self):
        ...
        btn_login.clicked.connect(self.perform_login)
...
# Form Area
        ...
        btn_layout.addWidget(self.resolve_btn) # Added here
    ...
    def mark_as_returned(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            item_id = self.table.item(current_row, 0).text()
            # Update status to "Returned" while keeping other data same
            self.db.update_item(
                item_id, 
